...
intents.guilds = True # Très important pour les commandes slash et les événements de guilde
...
def get_directory_size_bytes_sync(path):
    ...
    for dirpath, dirnames, filenames in os.walk(path):
        for f in filenames:
            ...
            try:
                if os.path.islink(fp): # Évite de suivre les liens symboliques pour éviter les boucles infinies ou les erreurs
                    continue
...
async def infos_tech_command(interaction: discord.Interaction):
    ...
    await interaction.response.defer(ephemeral=False) # Déferre la réponse pour les calculs longs
    ...
    if platform.system() == "Linux":
        try:
            ...
            try:
                ...
                linux_distro_info = ' '.join(filter(None, platform.linux_distribution())) # filter(None) pour enlever les éléments vides
                if not linux_distro_info: # Si linux_distribution() retourne vide ou n'est pas dispo
                    linux_distro_info = "Impossible de récupérer la distribution Linux (fallback)"
            except Exception as e:
                print(f"Erreur lors de la récupération de la distribution Linux (fallback): {e}")
    ...
    embed.set_footer(text=f"Dernière mise à jour: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
async def on_ready():
    ...
    try:
        ...
        synced = await bot.tree.sync()
        print(f"✅ Synchronisé {len(synced)} commandes slash globales après le démarrage.")
    except Exception as e:
        print(f"❌ Erreur lors de la synchronisation des commandes slash dans on_ready : {e}")
        traceback.print_exc()
